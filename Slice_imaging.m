function  [integrated_image, timecourse, soma, df_frames] = Slice_imaging(filename, autodetect, shortened)
...
% Adapted for 2p slice imaging by: Astra S. Bryant
% 5/2015
% Added features: detects sequence of image files, runs them. 
...
frames(nfo(1).Height, nfo(1).Width, 1:numel(nfo)) = uint16(ones);
...
frames=double(frames); %transform from uint16 to double
...
d_frames = (frames - baseline);  %calculate min-subtracted F
clear baseline

baseline= mean(d_frames(:,:,:),3);
baseline = repmat(baseline, [1, 1, size(d_frames, 3)]);

df_frames = ((d_frames - baseline)./baseline);%calculate dF/F
