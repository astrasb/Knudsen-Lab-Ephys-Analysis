    ...
    if isfield( TIF, 'MM_stack' )
        ...
        for ii = img_first:img_last
            ...
            %if ( TIF.SamplesPerPixel == 1 )
              %  IMG.data(1:IMG.width, 1:IMG.height,ii) =
              %  read_plane(fileOffset, IMG.width, IMG.height, 1);
               IMG.data(1:IMG.width, 1:IMG.height,1) = read_plane(fileOffset, IMG.width, IMG.height, 1);%EDITED from LINE ABOVE TO MAKE it only fill one img.data matrix (not the ii-th one) on 4/14/13
            %else
...
%                IMG.blue  = read_plane(0, IMG.width, IMG.height, 3);
                 for ii = img_first:img_last
                     %IMG.data(:,:,ii)=read_plane(0, IMG.width, IMG.height,
                     %ii);        
                     IMG.data(:,:,1)=read_plane(0, IMG.width, IMG.height, ii); %EDITED from LINE ABOVE TO MAKE it only fill one img.data matrix (not the ii-th one) on 4/14/13
                 end
            end
