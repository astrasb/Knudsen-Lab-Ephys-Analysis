function [frames sumstim avgstim] = OGBT(fileno, use_mn)
...
if exist('use_mn') == 0 || isempty('use_mn');
    use_mn = 0;
end
...
%fmax = max(max(max(frames(ROI, ROI, :))));
fmax = max(f1(f1>0));
fmin = min(f1(f1>0));
clear f1
f1 = uint8((frames-fmin)/(fmax-fmin)*255);      %(frames-min)/range

%% select the post-adaptation stims to make the image

%now highlight some ROIs.
%lasttrain = find(diff(stimtime) > 1);
%lasttrain = stimtime(lasttrain(1)+1);
%lasttrain = find(tms > lasttrain, 1);
lasttrain = find(tms > 2.5, 1);

im_sum = sum(f1(ROI, ROI, lasttrain:end), 3); %creates a summed image (not including the borders that might appear, depending on the scope
im_sum = (sum(f1(:, :, lasttrain:end), 3)-min(im_sum(im_sum > 0)))/range(im_sum(im_sum > 0))*255;   %normalizes frames with the min and range of the summed image

%im_mn = mean(f1(:, :, lasttrain:end), 3);

if use_mn == 1
    im_sum = mean(f1(:, :, lasttrain:end), 3);  %use the mean value, not the sum value
end
...
for f = 1:size(frames, 3)
    ...
    %determine if a stim is present
