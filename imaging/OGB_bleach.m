function [frames sumstim avgstim] = OGB_analysis(fileno, add_abf)
...
if add_abf == 1
    ...
    if ~isempty(a)  %if we have a hit, upload the abf file
        filename = abf(a).name; disp('Matching abf file found!'); disp(' ')
    else
        [filename] = uigetfile('*.abf', 'Pick the associated abf file');
    end
    [trace dt header] = abf2load(filename); sfq = 1/(dt*1e-6);
    ...
    stimtime = find(trace(:, stimchan)> 10, 1)/sfq;  %find the time of the pulse
    if isempty(stimtime)
    ...
    baseframe = find(tms > stimtime, 1) -2;     % find the time of the stimulus, and back up two frames to end the baseline period
else
...
for f = 1:size(frames, 3)
    ...
    if add_abf == 1
        text(nfo(1).Width-50, 25, num2str(tms(f)), 'Color', 'w', 'FontSize', 15); %add time stamp
    end
    currFrame = getframe;
