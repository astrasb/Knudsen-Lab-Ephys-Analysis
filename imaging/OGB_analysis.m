openfunction [frames sumstim avgstim] = OGB(fileno, add_abf)
...
if add_abf == 1
    ...
    [s stimtime] = findpeaks(abs(trace(:, stimchan)), 'minpeakheight', 10);  %find the time of the stimuli
...
zoom on
if add_abf == 1
    subplot(3, 2, 5:6); plot(tms, dFspot); legend(leg); xlabel('time (s)'); ylabel('dF/F (% percent fluorescence)');
