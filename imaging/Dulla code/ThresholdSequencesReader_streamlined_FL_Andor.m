...
path1=sprintf('%s/*.sif',directoryname);
d = dir (path1);
numfiles=length(d);
if numfiles<1
    disp('No files found');
...
for thisfile =1:numfiles     %%%%%  Controls which files are being analyzed
    ...
    [Image,InstaImage,CalibImage,vers]=andorread_chris_local_knownfilename(sprintf('%s//%s',directoryname,filename));
    temp=Image.data;
    exposuretime=InstaImage.exposure_time;
    pathd=sprintf('%s/*dark*',directoryname);
    ddark = dir (pathd);
    ...
    %[DarkImage,DarkInstaImage,DarkCalibImage,Darkvers]=andorread_chris_local_knownfilename(sprintf('%s/%s',directoryname,dddark(1,:)));
    ...
    pathc=sprintf('%s/*abf*',directoryname);
    dpclamp= dir (pathc);
    ...
    %% Get PCLAMP Data
    filenamepclamp=sprintf('%s/%s',directoryname,ddpclamp(1,:));
    ...
    TraceData=e_trace_clipped;
    FrameTimes=e_time_clipped;
    


    %%% Draw Mask of the Slice
    Images=Fluorescence_dark_clipped;
    if thisfile==1
        testimage=Images(1:size(Images,1)/2,:,500);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ch1=Images(1:(size(Fluorescence_dark_clipped,1)/2),:,:);
    ch2=Images((size(Fluorescence_dark_clipped,1)/2)+1:size(Fluorescence_dark_clipped,1),:,:);

    if 2>3
    %%%%%  Inactive Dual Channel Independent Curve Fitting
    %Curve Fitting
   ...
   Ratio=ch1./ch2;
    Ratio_to_fit=squeeze(mean(mean(Ratio)));
    %%%% Plotting the mean of Ratio Curve fitting
    ...
    fit_ratio=Ratio_to_fit(1:x_cutoff);
    ...
    fit_ratio=[fit_ratio;Ratio_to_fit(x_restart:size(ch1,3))];
    ...
    fit_ratio=double(fit_ratio);
    
    fit_time=fit_time';

    %%%% Fitting Ratio Average
    % --- Create fit "fit 1"
    fo_ = fitoptions('method','NonlinearLeastSquares');%,'Robust','On','Algorithm','Levenberg-Marquardt');
    ok_ = isfinite(fit_time) & isfinite(fit_ratio);
    if ~all( ok_ )
        warning( 'GenerateMFile:IgnoringNansAndInfs', ...
            'Ignoring NaNs and Infs in data' );
    end
    ...
    % Fit this model using new data
    cf_ = fit(fit_time(ok_),fit_ratio(ok_),ft_,fo_);
    ...
    for hh=1:size(ch1,3)
        Ratio_fit_results(hh)=coeffvalues1(1)*exp(coeffvalues1(2)*hh)+coeffvalues1(3)*exp(coeffvalues1(4)*hh);
    end

    Ratiosub=Ratio_to_fit-Ratio_fit_results';
    Ratiosub=Ratiosub+Ratio_to_fit(1,1);
    

    Ratiostart=Ratio(:,:,1);
    Ratioend=Ratio(:,:,size(ch1,3));
    


    %%% Subtractive normalization based on curve fit
    Ratiofitmax=max(Ratio_fit_results);
    
    Ratiofitmin=min(Ratio_fit_results);
    
    Ratiodiff=Ratiofitmax-Ratiofitmin;
    
    Ratio_Normalized=zeros(size(Ratio,1),size(Ratio,2),size(Ratio,3));
    
    for i=1:size(ch1,3)

        ThisFrame1=Ratio(:,:,i);
        TimeRelativeToStart1=(Ratio_fit_results(i)-Ratiofitmin)/Ratiodiff;
        TimeRelativeToEnd1=1-TimeRelativeToStart1;
        tempFrame1= Ratiostart*TimeRelativeToStart1+Ratioend*TimeRelativeToEnd1;
        NormFrame1=ThisFrame1-tempFrame1+mean(mean(mean(Ratiostart(size(ch1,1)/4:3*size(ch1,1)/4,size(ch1,2)/8:7*size(ch1,2)/8))));
        Ratio_Normalized(:,:,i)=NormFrame1;
    ...
    for timewindows=1:60
        ...
        [out_Images, Subtracted_Image]=Make_Composite_Image_RedShirt_Modifiable_non_cellarray(Ratio, TraceData, FrameTimes,InstaImage.integration_cycle_time*1000,-100+((timewindows-1)*10),-100+((timewindows)*10),stimframe );
