 function [d,si,sw,tags,et,cn,ep]=abfload2(fn,varargin)
...
%
% sw          scalar           the number of episodes (i.e., sweeps)
% tags        array            a two dimensional array of tag info	(three columns: time in sec, episode #, tag text)
% et          array            a one dimensional arrawy of episode times (sec)
% cn          cell array, in the same format as "channels", which is an optional input parameter.
% ep          structure        structure containing epoch duration, epoch start point, pulse duration, pulse width
%
...
headPar={
  ...
  'lEpochInitDuration', 2508, 'int32', repmat(-1,1,20);
  'lEpochPulsePeriod', 2136, 'int32', repmat(-1,1,20);
  'lEpochPulseWidth', 2216, 'int32', repmat(-1,1,20);
...
for i=1:size(recChNames,1)
    cn{i}=deblank(recChNames(i,1:10));
end
chInd=[];
eflag=0;
if ischar(channels) 
...
switch nOperationMode
  case 1
    ...
    % the byte offset at which the Synch
	ArraySection starts
  ...
  case {2,4,5} % winscanselect says that output files are mode 4, but really should be mode 5  for now, allow type 5 to fit in here 8/20/09 jrh, 
    if nOperationMode==2
      if verbose
...
%% syncharray reader, determine the time of each sweep from the start of the file

%%  'lSynchArrayPtr',92,'int32',-1;
%%  'lSynchArraySize',96,'int32',-1; 
if lSynchArraySize > 0	
    switch fSynchTimeUnit
      case 0  % time information in synch array section is in terms of ticks
        synchArrTimeBase=1;
      otherwise % time information in synch array section is in terms of usec
	...
	fseek(fid,lSynchArrayPtr*blockSz,'bof') ;
    [synchArr,n]=fread(fid,lSynchArraySize*2,'int32');
    if n~=lSynchArraySize*2,
      fclose(fid);
      error('something went wrong reading synch array section');
    ...
    % make synchArr a lSynchArraySize x 2 matrix
    synchArr=permute(reshape(synchArr',2,lSynchArraySize),[2 1]);
	et=synchArr(:,1)*(synchArrTimeBase*.000001);
end

%Tag Reader - Added by Max Kleiman-Weiner Summer 2007
...
if lNumTagEntries > 0	
    ...
    %% 'time',0,'tagcomment',{'testcomment'},'episode',0);
	fseek(fid,lTagSectionPtr*blockSz,'bof') ;
	for i = 1:lNumTagEntries
		tags(i).time = fread(fid, 1, 'int32') * fSynchTimeUnit * .000001 ;
		%%tags(i,2) 
         tags(i).comment = cellstr(fread(fid, ABF_TAGCOMMENTLEN, 'uint8=>char')') ;
		fread(fid,2,'int16');
		swp=-1;
		for j=1:sw
			if swp<0
...
%% Epoch time reader 
% - added 2009/07/17 by Christine Lee
% - This code may not work properly if there is an epoch duration increment ("delta duration")

% initial offset ("first holding period" GetHoldingLength in ABFHWAVE.CPP)
% double check this though
initOffset = round(lNumSamplesPerEpisode/nADCNumChannels /64);

epochDurations = reshape(lEpochInitDuration,10,2); % separate epoch durations by channel
epochStartPts = initOffset + [zeros(1,2); cumsum(epochDurations(1:end-1,:),1)]; % epoch start points (sample number) for each episode

ep.epochDuration = epochDurations; % epoch durations
ep.epochStartPts = epochStartPts; % epoch start point (within each episode) for epochs (0,9)
ep.epochPulsePeriod = reshape(lEpochPulsePeriod,10,2); % epoch pulse period
ep.epochPulseWidth = reshape(lEpochPulseWidth,10,2); % epoch pulse width
% epochStartTimes = epochStartPts/(1000/si);
