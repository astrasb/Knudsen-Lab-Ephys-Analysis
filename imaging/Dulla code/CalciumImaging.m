...
for thisfolder=1:folders;  %%%%%  Begin crawling through the folders 
    ...
    for thissif=1:sifs
        ...
        %%%%%%%%%%%% Open Parameter file
        ext='ameter';
        PathName=sprintf('%s/%s/',directoryname,dir_tree(thisfolder,1).name);
        ...
        if numfiles<1
            ...
            Roi_test=Parameters_in.parameters{5};
            Roi_test2=Parameters_in.parameters{6};
            Roi_test3=Parameters_in.parameters{7};
            x_cutoff=Parameters_in.parameters{8};
            x_restart=Parameters_in.parameters{9};
        end
        else
        parameters_present=1;
        end
...
[ratio, ratio_raw,Aligned,VertAdjust,HorizAdjust]=realignDVimages(ch1, ch2, Ch1_Normalized, Ch2_Normalized,Aligned,parameters_present,VertAdjust,HorizAdjust);  
        ...
        parameters{4}=mask;
        parameters{5}= Roi_test;
        parameters{6}= Roi_test2;
        parameters{7}= Roi_test3;
        parameters{8}=x_cutoff;
        parameters{9}= x_restart;
        parameterfile=sprintf('%sParameters',PathName);
        save (parameterfile,'parameters');
        text_f=sprintf('%s%s-%s_ROI',PathName,InstaImage.filename(52:61),InstaImage.filename(71:size(InstaImage.filename,2)-5));
