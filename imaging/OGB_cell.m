function [frames sumstim avgstim] = OGB_cell(fileno, shortened)
...
if add_abf == 1
    ...
    stimchan = find(strncmp(header.recChNames, 'holding', 7)>0);  %find the channel with stim.
      
    stim_cutoff = 0.9*max(trace(:, stimchan));
    stimtime = find((trace(:, stimchan)) > 1)/sfq;
        
    if isempty(stimtime)
        stimtime = input('Enter the time of the first depolarizing pulse (in sec). -->  ');
